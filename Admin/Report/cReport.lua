...
TicketUserColumnID = guiGridListAddColumn(Ticketgridlist, "ID", 0.1)
...
addEventHandler("doUserTickets", lp, UserloadTicketsystem);


addEventHandler("onClientGUIClick", TicketsendButton, function()
	local reason, text = tostring(guiGetText(TicketreasonEdit)), getPlayerName(lp)..': '..tostring(guiGetText(TickettextEdit))
	...
	if(#reason > 0 and #text > 0)then
		if(tostring(comboText) == "Fragen")then
			...
			guiSetVisible(Ticketwindow, false)
			triggerServerEvent("doSendTicket", lp, reason, text, "Fragen")
		elseif(tostring(comboText) == "Support - Hilfe")then
			setWindowData("remove","cursor_clicked",true)
			guiSetVisible(Ticketwindow, false)
			triggerServerEvent("doSendTicket", lp, reason, text, "Support")
		else
...
addEventHandler("onClientGUIClick", TicketcloseandDeleteButton, function()
	...
	if(id and id ~= -1)then
		if(#id > 0)then
			setWindowData("remove","cursor_clicked",true)
			guiSetVisible(Ticketwindow, false)
			triggerServerEvent("doDeleteTicket", lp, id)
		end
	end
end, false)
...
addEventHandler("onClientGUIClick", TicketopenAntwortenButton, function()
	...
	local endtext = before..'\n'..getPlayerName(lp)..': '..retext
	
	triggerServerEvent("doReplyTicket", lp, endtext, reid)
...
TicketAdminIDColumn = guiGridListAddColumn(TicketAdmingridlist, "ID", 0.1)
TicketAdminReasonColumn = guiGridListAddColumn(TicketAdmingridlist, "Reason", 0.3)
TicketAdminCatColumn = guiGridListAddColumn(TicketAdmingridlist, "Category", 0.3)
TicketAdminTimeColumn = guiGridListAddColumn(TicketAdmingridlist, "Date", 0.3)
TicketAdminStnColumn = guiGridListAddColumn(TicketAdmingridlist, "State", 0.2)
	...
	if(id and id ~= -1)then
		if(#id > 0)then
			...
			if(combotext == "Antworten")then
				...
				local retext  = before..'\n'..getPlayerName(lp)..": "..regain
				triggerServerEvent("doReplyTicket", lp, retext, id);
			elseif(combotext == "Antworten + Schließen")then
				...
				local retext  = before..'\n'..getPlayerName(lp)..": "..regain
				triggerServerEvent("doReplyTicketClose", lp, retext, id);
			elseif(combotext == "Schließen")then
				setWindowData("remove","cursor_clicked",true)
				guiSetVisible(TicketAdminwindow, false)
				triggerServerEvent("doTicketClose", lp, id);
			end
		end
	end
end, false)
...
addEventHandler("doAdminTickets", lp, function(atickets)
