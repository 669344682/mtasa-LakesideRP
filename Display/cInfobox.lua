...
local sound=nil
function Infobox:create(typ,msg,time,title)
	if(typ)then
		if(typ=="error")then
...
function Infobox:draw(delta)
    ...
    for idx,box in ipairs(self.m_Boxs)do
        if(box.stage==1)then
            local alpha,posX,_=interpolateBetween(box.alpha,box.posX,0,self.m_Alpha,self.m_PosX,0,self:getProgress(idx),"OutBounce")
        ...
        if(box.stage==3)then
            local alpha,posY,_=interpolateBetween(box.alpha,box.posY,0,0,-self.m_Height-5,0,self:getProgress(idx),"OutBounce")
