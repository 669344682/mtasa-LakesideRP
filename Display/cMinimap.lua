				...
				if getElementData(lp,"intchange")==false then
					--if getElementData(lp,"introstate")==false then
						if getElementData(lp,"inRocket")==false then
							if getElementData(lp,"onTheMoon")==false then
									...
									if (not isPlayerMapVisible()) then
										...
										if alwaysRenderMap or getElementInterior(lp) == 0 then
											...
											dxDrawImage(X - worldW/2, mH/5 + (Y - worldH/2), worldW, worldH, "Files/Images/Radar/Map"..syncClientGetElementData("Radar")..".png", camZ, (x/(6000/worldW)), -(y/(6000/worldH)), tocolor(255, 255, 255, 255))
										end
										dxSetRenderTarget()
										...
										for k,v in ipairs(getElementsByType("blip"))do
											...
											if actualDist <= maxDist and getElementDimension(v)==getElementDimension(lp) and getElementInterior(v)==getElementInterior(lp) then
												...
												dxDrawImage(bpx -(blip*bS)*xFactor/2, bpy -(blip*bS)*yFactor/2, (blip*bS)*xFactor, (blip*bS)*yFactor, "Files/Images/Radar/Blips/"..icon..".png", 0, 0, 0, tocolor(bcR, bcG, bcB, bcA))
											end
										end
										if renderNorthBlip then
											...
											if bpx and bpy then
												local bpx = math.max(lB, math.min(rB, bpx))
												local bpy = math.max(tB, math.min(bB, bpy))
												dxDrawImage(bpx -(blip*2)/2, bpy -(blip*2)/2, blip*2, blip*2, "Files/Images/Radar/Blips/4.png", 0, 0, 0)
											end
										end
										dxDrawImage(cX -(blip*2)*xFactor/2, cY -(blip*2)*yFactor/2, (blip*2)*xFactor, (blip*2)*yFactor, "Files/Images/Radar/Blips/2.png", camZ-rz, 0, 0)
									end
							end
						end
					--end
